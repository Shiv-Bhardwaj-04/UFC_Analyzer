...
def render(fighters_df, search_engine):
    ...
    <div class='info-box'>
        ...
        <p><strong>Examples:</strong> "Jon", "Bones", "Holloway", "Cerrone", "Cowboy", etc.</p>
    </div>
    ...
    with col1:
        if search_method == "🔍 Smart Search":
            query = st.text_input(
                ...
                placeholder="e.g., Jon Jones, Bones, Holloway, Cerrone, Cowboy...",
                key="smart_search"
            )
            ...
            if query:
                ...
                if len(results) > 0:
                    ...
                    st.info("""
                    **Search Tips:**
                    - Try searching by first name only (e.g., "Jon")
                    - Try searching by last name only (e.g., "Cerrone")
                    - Try the fighter's nickname (e.g., "Cowboy" or "Blessed")
